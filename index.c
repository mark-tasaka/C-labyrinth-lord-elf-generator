...
char* primeReqBonus(unsigned strength, unsigned intelligence);
...
int main(void)
{
	/*Holds the elf's name*/
	char elfName[ARRAY_SIZE];

	/*Holds the elf's alignment*/
	...
	/*Stores elf class bonuses*/
	char elfBonuses[ARRAY_SIZE] = "\nInfravision of 60'.\n2 in 6 chance to detect hidden or secret doors.\nImmune to paralysis ghouls can inflict.";

	/*Save file name*/
	char saveFile[ARRAY_SIZE] = "";

	/*Introduction text*/
	printf_s("Welcome to the Labyrinth Lord Elf Generators.\n", ARRAY_SIZE);

	/*prompt user to enter elf's name*/
	printf_s("\nEnter the name of your Elf: \n", ARRAY_SIZE);

	scanf_s("%[^\n]s", elfName, ARRAY_SIZE);
	...
	do {
		...
		/*if statement used to ensure intelligence is minimum of 9 as elf
		class requirement*/
		if (intelligence < 9)
		{
			intelligence = 9;
		...
		strcpy(primeRequisiteBonus, primeReqBonus(strength, intelligence));
		...
		printf_s("\nNotes: %s%s%s", primeRequisiteBonus, charismaReaction, elfBonuses, ARRAY_SIZE);
	...
	printf_s("\nYou have %u gold pieces (gp) to spend on your elf.  Now it is time to buy some armour, weapons and gear.\n\n", gold, ARRAY_SIZE);
	...
	/*Option of purchasing a shield if elf has at least 10 gp*/
	if (gold >= 10)
	{
	...
	printf_s("\nNow, let's purchase some weapons for your elf.\n", ARRAY_SIZE);

	/*Function for adding weapons for elf*/
	do
	{
		choice = 0;
	...
	/*Function for adding gear for elf*/
	do
	{
		choice = 0;
	...
	printf_s("\nDo you want to save your elf (%s) to a text files?\n1.yes\n2.no\n(please select 1 (yes) or 2 (no)\n", elfName, ARRAY_SIZE);

	scanf(" %d", &noYes);

	if (noYes == 1)
	{
		printf_s("\nPlease a name (no spaces, please) for the file you want to save the elf's information to: ", ARRAY_SIZE);
...
/*Randomly generates the elf's hit points
...
/*displays the prime requisite bonus for elf*/
char* primeReqBonus(unsigned strength, unsigned intelligence)
{
	char primeReq[ARRAY_SIZE] = " ";

	if (strength < 13 && intelligence >= 13)
	{
		strcpy(primeReq, "\n+5% Experience Adjustment");
	}
	else if (strength >= 13 && intelligence < 13)
	{
		strcpy(primeReq, "\n+5% Experience Adjustment");
	}
	else if (strength >= 13 && intelligence >= 13)
